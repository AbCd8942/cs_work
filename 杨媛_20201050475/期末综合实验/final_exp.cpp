...
void Draw_Triangle()
{
	GLfloat mat_specular[] = { 0.6f, 0.6f, 0.6f, 1.0f };//颜色
	GLfloat mat_diffuse0[] = { 0.65f, 0.65f, 0.2f, 1.0f };
	GLfloat mat_diffuse1[] = { 1.0f, 1.0f, 1.0f };
	GLfloat mat_diffuse2[] = { 0.0f, 1.0f, 1.0f };
	GLfloat mat_diffuse3[] = { 1.0f, 0.0f, 0.0f };
	GLfloat mat_diffuse4[] = { 0.0f, 1.0f, 0.0f };
...
void key(unsigned char k, int x, int y)
{
	...
	{
	case 27:
	case 'q': {exit(0); break; }//退出程序
	case 'p': {bPersp = !bPersp; break; }//缩放
	case ' ': {bAnim = !bAnim; break; }//旋转
	case 'o': {bWire = !bWire; break; }//线框图
	case 'a': { //整体左移
		eye[0] += 0.2f;
		center[0] += 0.2f;
		break;
...
int main (int argc, char *argv[])
{
	...
	int windowHandle = glutCreateWindow("茶壶与光照");//设置窗口标题
